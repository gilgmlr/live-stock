                ...
                for($row=3; $row<=$highestRow; $row++){
                    if ($worksheet->getCellByColumnAndRow(0, $row)->getValue() != null) {
                        if ($table == 'warehouse') {
                            ...
                            $res_inven = $this->db->get_where('inventory', [
                                'item_code' => $worksheet->getCellByColumnAndRow(0, $row)->getValue(), 
            ...
            foreach($object->getWorksheetIterator() as $worksheet){
                ...
                for($row=3; $row<=$highestRow; $row++){
                    if ($worksheet->getCellByColumnAndRow(0, $row)->getValue() != null) {
                        ...
                        $this->db->set('stocks', $worksheet->getCellByColumnAndRow(2, $row)->getValue());
                        $this->db->where('item_code', $item_code);
                        $this->db->where('location', $location);
                        $this->db->update('inventory');
                        // $this->db->update_batch('inventory', $data, 'item_code');
                        
                    }
                } 
            }
 
            // $this->db->update_batch($table, $data, 'item_code');
            // $this->db->insert_batch("material_issue", $mi);
            $this->session->set_flashdata('flash', 'Import Update File Excel Saved in database ' . $table);
            redirect('settings/view_import_data');
        } else {
            $this->session->set_flashdata('flash', 'Import Update File is Failed, Your file type not excel!');
            redirect('settings/view_import_data');
        }
    }
}